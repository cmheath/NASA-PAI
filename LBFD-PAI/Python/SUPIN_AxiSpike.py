...
from Helper import copy_files, remove_files, _2str
...
class Capture(VariableTree):
    ...
    FAcap = Float(0.0, desc = '')
    Fhclip = Float(0.0, desc = '')
    ...
    ARbotcap = Float(1.0, desc = '')
    pbotcap = Float(2.0, desc = '')

class ExtSupDiff(VariableTree):
    DataID = 4

    Kexd = Int(5, desc = 'Flag input for how the external diffuser geometry is created')
    Kmatch = Int(1, desc = 'Flag indicating the flow condition to match at the supersonic outflow, 0=Do not match flow condition and use stage angles explicitly, 1=Fmatch is the Mach No., 2=Fmatch is the pressure ratio Pex/PeL, 3=Fmatch is flow angle at station EX (degrees)')
    Fmatch = Float(1.3, low = 0.0, desc = 'Flow condition to be matched at diffuser outflow')
    Kfocal = Int(1, desc = 'Flag for how focal points are used, 1=Shock on lip position, 2=Use the first stage offset')
    Tcap = Int(1, desc = '')
    Knose = Int(1, desc = 'Flag for the shape of the nose, 1=sharp, 2=circular')
    Xnose = Float(0.0, desc = 'X-coordinates for the start of the nose', units = 'ft')
    Rnose = Float(0.0, desc = 'Radius of the nose', units = 'ft')
    Stage = Array(array([[1, 2, 3]]), dtype=Int, shape=(1,3), desc = 'Stage number')
    Angle = Array(array([[15.0, 18.4635, 22.3113]]), dtype=Float, shape=(1,3), desc = 'Slope angle for the surface through a stage')    
    Xstage = Array(array([[-1.47038, -0.60311, 0.39883]]), dtype=Float, shape=(1,3), desc = 'Axial x-coordinate at the end of a stage', units = 'ft')
    Dyfocal = Array(array([[0.0, 0.0, 0.0]]), dtype=Float, shape=(1,3), desc = 'Offset in y-direction from yclip for a focal point for a stage')    
...
class CowlLip(VariableTree):
    ...
    thclin = Float(10.0, desc = 'Angle of the cowl lip interior', units = 'deg')
    ...
    thclex = Float(12.5, desc = 'Angle of cowl lip exterior', units = 'deg')
...
class Throat(VariableTree):
    ...
    Kthrt = Int(5, desc = 'Flag for basic shape of the throat, 1=Simple throat model, 2=Zero-length throat, 3=Straight, constant area throat, 4=Contoured cowl throat for axisymmetric pitot inlet, 5=Single-stage throat with centerbody and turning, 6=Two-stage throat with a centerbody and turning, 7=Supersonic throat for streamline-traced inlets, 8-Subsonic throat model for streamline-traced inlets')
    Kthcb = Int(1, desc = 'Flag for defining planar profile of cowl throat, 0=undefined, 1=use geometry specified by NURBS, 2=Use specified set of centerbody entities')
    dthcb1 = Float(0.0, desc = 'Break in slope of the centerbody at station 1', units = 'deg')   
    dxcbSD = Float(1.5, desc = 'Axial distance on centerbody from point cb1 to point cbSD', units = 'ft')
    dycbSD = Float(0.0, desc = 'Transverse distance on centerbody from point cb1 to point cbSD', units = 'ft')
    thcbSD = Float(-10.0, desc = 'Slope of the centerbody at station SD', units = 'deg')
    Fncb1 = Float(0.333, desc = 'Factor for NURBS interior control point at station 1', units = 'ft')
    FncbSD = Float(0.333, desc = 'Factor for NURBS interior control point at station SD')
    Kthcw = Int(1, desc = '', units = 'deg')
    aTHa1 = Float(1.07, desc = '')    
    thcwTH = Float(0.0, desc = '')
    machTH = Float(-0.75, desc = '')
    aSDa1 = Float(1.14, desc = '')
    thcwSD = Float(-2.0, desc = '')
    machSD = Float(-0.6, desc = '')

class SubDiff(VariableTree):
    DataID = 10

    Ksubd = Int(3, desc = 'Flag for the basic shape of the subsonic diffuser, 0=Undefined, 1=Simple subsonic diffuser model, 2=Axisymmetric cowl with no centerbody, 3=Axisymmetric cowl and centerbody, 4=Diffuser for a 2-D inlet with single duct, 5=Diffuser for a 2-D bifurcated inlet, 6=Diffuser defined by super-ellipses placed along a guiding curve, 7=Diffuser defined by curves at each circular coordinate')
    ...
    Ksdcb = Int(1, desc = 'Flag for duffuser of subsonic cowl profile, 1=geometry model based on 4-pt NURBS')
    FdcbSD = Float(0.333, desc = 'Factor for NURBS interior control point at station SD')
    FdcbX = Float(0.05, desc = 'Factor for NURBS interior control point at start of the straight section')
    thcbX = Float(-5.0, desc = 'Fraction of subsonic diffuser length for the straight section on the cowl')       
    FLcbX = Float(0.6, desc = 'Fraction of subsonic diffuser length for the straight section on the cowl')
    Ksdcw = Int(1, desc = 'Flag for diffuser of subsonic cowl profile, 1=geometry model based on 4-pt NURBS')
    ...
    FLcwX = Float(0.1, desc = 'Fraction of subsonic diffuser length for the straight section on the cowl')         
...
class EngineFace(VariableTree):
    ...
    Kspin = Int(0, desc = 'Flag for the shape of the nose of an axisymmetric spinner, 0=No spinner, 1=Sharp, 2=Circular, 3=Elliptical')
...
class SUPIN(ExternalCode):
    ...
    Ktype = Int(3, desc = 'Type of inlet geometry being designed, 1=2D Axi Pitot, 5=STEX, 6=3D Pitot')
    ...
    FWinp = Float(51.944, desc = 'Input flow rate', units = 'lbm/s')
    ...
    def __init__(self, *args, **kwargs):
        ...
        copy_files(os.path.join(os.getcwd(), '../ESP/AxiSpike/*'), os.path.join(os.getcwd(), '../ESP/'))
