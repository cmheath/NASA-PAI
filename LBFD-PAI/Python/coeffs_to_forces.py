...
with open('LBFD.forces.3') as f:
    for bndry in IDs:
        ...
        print ("%s  L = %s, D = %s, Fx = %s, Fz = %s\n" % (bndry.ljust(10), str(L).ljust(10), str(D).ljust(10), str(Fx).ljust(10), str(Fz).ljust(10)))

        f.seek(0,0)

print ("Net Lift = %.6f lbf     Net Drag = %.6f lbf\n" % (L_net, D_net))
#print ("Net Fx = %.6f     Net Fz = %.6f\n" % (Fx_net, Fz_net))
