...
class Freestream(VariableTree):
    ...
    Kfs = Int(2, desc = 'Flag to specify freestream conditions, 1=std. atm, 2=static P and T, 3=Total P and T, 4=Dynamic P and T')
...
class ExtSupDiff(VariableTree):
    ...
    Fstle = Float(-4.5, desc = 'Angle of supersonic diffuser leading edge')
    ...
    stpbot = Float(2.2, desc = 'Super-ellipse parameter for the bottom tracing cross-section (>=2.0)')
    stptop = Float(1.9, desc = 'Super ellipse parameter for the the top tracing cross-section (>=2.0)')    
...
class CowlLip(VariableTree):
    ...
    bclin = Float(0.0004, desc = 'Radius of length of semi-minor axis for cowl lip interior', units = 'ft')
    ...
    bclex = Float(0.0004, desc = 'Radius of length of semi-minor axis for cowl lip exterior', units = 'ft')
...
class CowlExt(VariableTree):
    ...
    Kcex = Int(3, desc = 'Methods for constructing the cowl exterior geometry, 0=No cowl exterior, 1=Create cowl exterior using planar curve model, 2=Create cowl exterior using a horizontal line from cowl lip exterior, 3=Create the cowl exterior using entities')
...
class Throat(VariableTree):
    ...
    Frstsh = Float(0.005, desc = 'Adjustment to slope at the shoulder at the symmetry plane, If Kstsh=1, Frstsh is desired slope at shoulder, If Kstsh=2, Frstsh is angle increment to adjust slope from MOC solution')

    stclphi = Float(120.0, desc = 'Circumferential extent of subsonic cowl lip (<=0 for no subsonic cowl lip)', units = 'deg')
    Fstcldx = Float(0.275, desc = 'Axial displacement of the subsonic cowl lip at the symmetry plane', units = 'ft')
    Fstcldy = Float(0.0, desc = 'Vertical displacement of the subsonic cowl lip at the symmetry plane', units = 'ft')
    stclth = Float(0.0, desc = 'Angle of incidence of the subsonic cowl lip at the symmetry plane', units = 'deg')
    stclb = Float(0.001, desc = 'Length of the semi-minor axis for the elliptical profile of the subsonic cowl lip', units = 'ft')
    stclar = Float(4.0, desc = 'Aspect ratio of the elliptical profile at the subsonic cowl lip')

    aSDa1 = Float(1.01, desc = 'Ratio of the cross-sectional area at station SD to area at station 1')    
    FdxSD = Float(0.1, desc = 'x-coordinate of station SD', units = 'ft')
    FdySD = Float(0.0, desc = 'y-coordinate of station SD', units = 'ft')
    pbotSD = Float(2.2, desc = 'Super-ellipse parameter for bottom curve of cross-section at station SD')
    ptopSD = Float(1.9, desc = 'Super-ellipse parameter for top curve of cross-section at station SD')
...
class SubDiff(VariableTree):
    ...
    FLsubd = Float(4.75, desc = 'Input defining length of the subsonic diffuser based on KLsubd')
...
class EngineFace(VariableTree):
    ...
    FyEF = Float(0.685, desc = 'Input for the vertical position of the center of the engine face', units = 'ft')
